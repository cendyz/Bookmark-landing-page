<template>
	<section class="px-[2rem] py-[4rem] dark:bg-gray-500">
		<div class="container">
			<RouterLink
				to="/"
				class="bg-gray-50 dark:bg-gray-400 dark:text-gray-100 py-[.4rem] px-[2rem] inline-flex items-center gap-x-[1rem] myShadow rounded-md"
				><Icon
					name="material-symbols:line-start-arrow-notch-rounded"
					class="text-black text-[2rem] dark:text-gray-50" />
				Back</RouterLink
			>
			<div class="mt-[6rem] text-gray-400 dark:text-gray-50">
				<img
					:src="actualCountry[0].flags.png"
					:alt="`${actualCountry[0].name} flag`"
					class="w-full h-[23rem] myShadow2 max-w-[35rem]" />
				<h1 class="font-w800 mt-[5rem] text-[2.5rem]">{{ actualCountry[0].name }}</h1>
				<div class="mt-[2.5rem] grid gap-y-[1.3rem] dark:text-gray-100">
				...
				<div>
					...
					<div class="flex flex-wrap items-center justify-center gap-[1rem] mt-[1.5rem]">
						<RouterLink
							...
							:key="index"
							class="py-[.3rem] text-center bg-gray-50 dark:bg-gray-400 myShadow rounded-md text-[1.3rem] w-[30%]"
